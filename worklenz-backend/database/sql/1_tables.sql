...
-- External and CSV import jobs
CREATE TABLE IF NOT EXISTS import_jobs (
    id                UUID                     DEFAULT uuid_generate_v4() PRIMARY KEY,
    provider          TEXT                                                NOT NULL,
    flow_type         TEXT                                                NOT NULL CHECK (flow_type IN ('direct', 'csv')),
    status            TEXT                     DEFAULT 'pending'           NOT NULL CHECK (status IN ('pending', 'ready', 'running', 'success', 'failed')),
...
CREATE TABLE IF NOT EXISTS import_stage_tasks (
    ...
    attachments_planned BOOLEAN                  DEFAULT FALSE NOT NULL,
    raw                 JSONB,
    created_at          TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL
);
